var webside = {
        ...
        initMenu : function() {
            $("[nav-menu]").each(function() {
                $(this).bind("click", function() {
                    ...
                    var sn = nav.split(",");
                    var snurl = sn[sn.length - 1];
                    if( snurl == '') return;//前台nav-menu属性是否存在判断逻辑在这里实现
                    if (snurl == '/sirona' || snurl == '/druid') {
                    	//if(snurl == '/druid'){	window.open(sys.rootPath + 'druid');} //url中两个//导致druid登录页面跳转到项目登陆页面
                    	//else{	window.open(sys.rootPath + snurl);}
                    	window.open(sys.rootPath + snurl);
                        //处理目录类型的点击事件
                        ...
                        //加载页面
                        $(".page-content").load(sys.rootPath + snurl);
                        //处理目录类型的点击事件
                        $(this).parent("li").siblings().find("ul.nav-show").removeClass('nav-show').addClass('nav-hide').attr('style', 'display:none');
                        //处理菜单类型的点击事件
                        $(this).parent().parent().parent("li").siblings().find("ul.nav-show").removeClass('nav-show').addClass('nav-hide').attr('style', 'display:none');
    ...
    common : {
         ...
         * 返回列表页面
         * 适用场景：编辑页面返回按钮点击事件
         * @param {Object} gobackUrl 转向的列表页地址
         * @param {Object} entity 页面实体是否存在（区分新增、编辑）
         */
        goback:function(gobackUrl, entity)
        {
    		if(entity){
    			gobackUrl += "?page="+$("#pageNum").val()+"&rows="+$("#pageSize").val()
    					+"&sidx="+$("#orderByColumn").val()+"&sord="+$("#orderByType").val();
    		}
    		webside.common.loadPage(gobackUrl);
        }
    },
    form : {
        user : {
